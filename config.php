 ...
 *
 * File created/changed: 2018-07-05T17:28:34+02:00
 */
...
function AdminThemeUikitConfig(AdminTheme $adminTheme, InputfieldWrapper $inputfields)
{
	...
	$fieldset->label = __('Theme Options');
	...
	$f->attr('name', 'variant');
	$f->label = __('Theme Color');
	$f->addOption('pw', __('ProcessWire blue'));
	$f->addOption('vibrant', __('Vibrant blue'));
	$f->addOption('black', __('Black'));
	$f->attr('value', $adminTheme->get('variant'));
	$f->icon = 'star';
	$fieldset->add($f);
	...
	if ($lastLayout != $layout) {
		...
		$o .= '[/script]';
		$f->appendMarkup = str_replace(['[', ']'], ['<', '>'], $o);
	}

	if (empty($_POST)) {
		$session->setFor($adminTheme, 'lastLayout', $layout);
	}
	...
	ksort($types);
	$skipTypes = ['Button', 'Submit', 'Form', 'Hidden'];
